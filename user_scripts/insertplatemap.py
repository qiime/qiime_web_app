...
script_info['script_description'] = """Input american gut plate map, sql output file"""
script_info['script_usage'] = [("Examples:","create db insert statment","%prog -i ag_plate_map.txt -o ag_plate_inserts.sql")]
script_info['output_description']= "sql file"
script_info['required_options'] = [\
 # Example required option
make_option('-i','--input_fp',type="existing_filepath",help='the input filepath'),\
make_option('-o','--output_fp', type="new_filepath",help='the output filename')\
]
...
def main():
    ...
    for line in platemaplines:
        ...
        if 'Plate' in line[0] and line[1] != '1':
            ...
            platename=line[0]
            sqlstmt = "insert into plate (plate_id, plate) values (seq_plate_id.nextval, '%s');\n" % platename
                ...
                for barcode in line[1:]:
                    if barcode != '':
                        ...
                        if barcode > 1000:
                            ...
                            sqlstmt = "insert into plate_barcode (plate_id, barcode) select plate_id, '%09d' from plate where plate = '%s';\n" % (barcode, platename)
                            sqloutfile.write(sqlstmt)
        else:
