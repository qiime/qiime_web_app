...
the american gut results directory and outputs an sql file to update the database"""
script_info['script_usage'] = [("Examples:","create database update statements","%prog -i listoffiles.txt -o updateresults.sql")]
script_info['output_description']= "an sql file to update results read status of db"
script_info['required_options'] = [\
 # Example required option
 make_option('-i','--input_fp',type="existing_filepath",help='the input filepath a list of all the files in the results directory'),\
...
def main():
    ...
    filenamesfilename=opts.input_fp
    filenamesfile = open(filenamesfilename, 'r')
    sqlfile=opts.output_fp
    sqloutfile=open(sqlfile, 'w')
    #barcodeline = barcodeslist.readline()
    barcodeline = filenamesfile.readline()
    while (barcodeline):
        barcode = barcodeline.split(' ')[-1].split('.')[0]
        sqlstmt = "update ag_kit_barcodes set results_ready = 'Y' where barcode = '%s';\n" % barcode
        sqloutfile.write(sqlstmt)
        sqlstmt = "update barcode set sequencing_status = 'SUCCESS' where barcode = '%s';\n" % barcode
        sqloutfile.write(sqlstmt)
        barcodeline = filenamesfile.readline()
    sqloutfile.close()
    filenamesfile.close()
