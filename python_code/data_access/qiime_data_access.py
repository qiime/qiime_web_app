...
class QiimeDataAccess(object):
    ...
    def getSampleDetailList(self, study_id):
        ...
        sample_details = []
        for sample_name, sample_id, public, collection_date, run_prefix, sequence_count, otu_count in results:
            sample_details.append((sample_name, sample_id, public, collection_date, run_prefix, sequence_count, otu_count))
    ...
    def updateSeqOtuCounts(self, sequence_prep_id, num_sequences, num_otus):
        """ Updates sequence and otu counts for a prep entry
        """
        try:
            con = self.getMetadataDatabaseConnection()
            results = con.cursor()
            con.cursor().callproc('update_seq_otu_counts', [sequence_prep_id, num_sequences, num_otus])
        except Exception, e:
            print 'Exception caught: %s.\nThe error is: %s' % (type(e), e)
            return False  
