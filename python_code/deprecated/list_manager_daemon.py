...
class ListManagerDaemon(Daemon):
    ...
    def checkListValue(self, list_name, value):
        ...
        if list_name not in self._lists:
            self._lists[list_name] = data_access_factory(ServerConfig.data_access_type).getListValues(list_name)
    ...
    def checkOntologyValue(self, ontology_name, term):
        ...
        if ontology_name not in self._ontologies:
            self._ontologies[ontology_name] = data_access_factory(ServerConfig.data_access_type).getOntologyValues(ontology_name)
