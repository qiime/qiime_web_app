
--------------------------------------------------------
--  DDL for Procedure GET_TEST_SPLIT_LIB_DATA
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "SFF"."GET_TEST_SPLIT_LIB_DATA" 
/* 
This procedure returns a set of data given an analysis id for testing 
purposes. 
*/
(
  -- define input/output variable for the procedure
  test_analysis_id IN NUMBER,
  test_sample IN VARCHAR2,
  user_data in out types.ref_cursor
) as 
begin
  BEGIN
    -- get a set of data for validation that the input data equals the data 
    -- in the DB
    open user_data for
      select j.seq_run_id,slrm.ssu_sequence_id,l.command,l.md5_checksum,
      s.sequence_string,s.md5_checksum
      from analysis j
      inner join read_454 r on j.seq_run_id=r.seq_run_id
      inner join split_library_read_map slrm on r.seq_run_id=slrm.seq_run_id and r.read_id=slrm.read_id
      inner join ssu_sequence s on slrm.ssu_sequence_id=s.ssu_sequence_id
      inner join split_library_run l on j.split_library_run_id=l.split_library_run_id
      where j.analysis_id=test_analysis_id and slrm.sequence_name=test_sample;
  END;
end get_test_split_lib_data;

/*
variable user_data REFCURSOR;
execute get_test_split_lib_data(455,'test_PCx634_1',:user_data);
print user_data;
*/